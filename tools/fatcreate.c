/* from https://github.com/LittleFox94/lf-os_amd64/blob/main/util/fatcreate.c */
/* modified to work with macos */
...
void stat2fat(struct directory_entry* entry, struct stat* stat) {
    struct tm ctime;
    localtime_r(&stat->st_ctimespec.tv_sec, &ctime);
    ...
    struct tm atime;
    localtime_r(&stat->st_atimespec.tv_sec, &atime);
    ...
    struct tm mtime;
    localtime_r(&stat->st_mtimespec.tv_sec, &mtime);
...
int main(int argc, char* argv[]) {
    ...
    memcpy(ctx.superblock.oem, "NOS     ", 8);
